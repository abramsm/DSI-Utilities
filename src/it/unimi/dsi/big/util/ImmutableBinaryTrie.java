...
public class ImmutableBinaryTrie<T> extends AbstractObject2LongFunction<T> implements Serializable {
	 ...
	 * that start with <code>word</code> (thus, the {@linkplain LongIntervals#EMPTY_INTERVAL empty inteval}
	 ...
	 * @return  an interval given by the smallest and the largest word in the trie 
	 * that start with the word returned by <code>iterator</code> (thus, the {@linkplain LongIntervals#EMPTY_INTERVAL empty interval}
	 * if no such words exist).
