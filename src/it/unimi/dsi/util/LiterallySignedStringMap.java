...
public class LiterallySignedStringMap extends AbstractObject2LongFunction<CharSequence> implements StringMap<MutableString>, Serializable {
	...
	public static void main( final String[] arg ) throws IOException, JSAPException, ClassNotFoundException, SecurityException, NoSuchMethodException {
		...
		ObjectList<MutableString> list = text ? new FileLinesCollection( listName, encoding.toString(), zipped ).allLines() : (ObjectList<MutableString>)BinIO.loadObject( listName );
		
		LOGGER.info( "Signing..." );
		BinIO.storeObject( new LiterallySignedStringMap( (Object2LongFunction<CharSequence>)BinIO.loadObject( functionName ), list ), mapName );
