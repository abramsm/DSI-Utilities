...
public class Jackknife {
	...
	public static Jackknife compute( final List<double[]> samples, final Statistic f, final MathContext mc ) {
		...
		for( double[] sample: samples ) { 
			...
			for( int p = l; p-- != 0; ) sum[ p ] = sum[ p ].add( BigDecimal.valueOf( sample[ p ] ) );
		}

		final BigDecimal[] averagedSample = new BigDecimal[ l ];
		for( int p = l; p-- != 0; ) averagedSample[ p ] = sum[ p ].multiply( big1OverN );
		...
		for( int s = 0; s < n; s++ ) {
			...
			for( int p = l; p-- != 0; ) leaveOneOutSample[ p ] = sum[ p ].subtract( BigDecimal.valueOf( t[ p ] ) ).multiply( big1OverNMinus1 );
		...
		for( int i = k; i-- != 0; ) {
			BigDecimal e = BigDecimal.valueOf( n ).multiply( naiveStatistics[ i ] );  
			for( int s = n; s-- != 0; ) e = e.subtract( leaveOneOutStatistic[ s ][ i ].multiply( bigNMinus1OverN ) );
			estimate[ i ] = e;

			BigDecimal variance = BigDecimal.ZERO;
			for( int s = n; s-- != 0; ) {
				final BigDecimal t = naiveStatistics[ i ].subtract( leaveOneOutStatistic[ s ][ i ] );
				variance = variance.add( t.multiply( t ) );
			}
			standardError[ i ] = Math.sqrt( variance.multiply( bigNMinus1OverN ).doubleValue() );			
		}
